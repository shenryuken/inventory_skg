        ...
        try{
            ...
            if($stock_adjustments->isNotEmpty()){
                foreach($stock_adjustments as $a => $b)
                {
                    ...
                    $supplier_name = isset($b->StockItem->where('stock_adjustment_id',$b->id)->first()->suppliers->company_name) ? $b->StockItem->first()->where('stock_adjustment_id',$b->id)->suppliers->company_name : "";
                    $supplier_code = isset($b->StockItem->where('stock_adjustment_id',$b->id)->first()->suppliers->supplier_code) ? $b->StockItem->first()->where('stock_adjustment_id',$b->id)->suppliers->supplier_code : "";
